# GA4 — Part-2: One Deployed App for 3 Questions (Q3, Q4, Q5)

You deploy **one** FastAPI app to Render (or any hosting) and submit its URL for three
...
The app is **fully self-contained** — Q4 data is generated in-memory from your email
using a Python port of the original JS seeded RNG. No CSV or JSON files needed.

## Step 0 — Create a New Repo & Upload Files

1. Go to GitHub and create a new public or private repository.
2. Create or upload all 3 files (given below in Step 1) to your new GitHub repository.

---

## Step 1 — Create the 4 files
1. config.py (input your mail id and api key in it)
2. main.py
3. requirements.txt
4. Dockerfile
...
1. Go to https://render.com/ and log in with your email or GitHub account.
2. Click **New** → **Web Service**.
3. Connect your GitHub repo. (If it's public, the URL is enough; if private, connect your GitHub account.)
...
7. Your base URL is shown at the top left: `https://<your-app-name>.onrender.com`

---

## Step 3 — Submit (use the map)

| Q      | Submit exactly                                           |
| :----- | :------------------------------------------------------- |
| **Q3** | `https://<your-app-name>.onrender.com/grounded-answer`   |
| **Q4** | `https://<your-app-name>.onrender.com/vector-search`     |
| **Q5** | `https://<your-app-name>.onrender.com` (base URL)        |
...
- **Check you api key:** Sign in and generate key till you see weekly token limit on dashboard of aipipe.org.
- **Q4 data is generated in memory** at startup from your `config.EMAIL` — no external files needed.
  If the grader sends a different email, the app always returns the correct student-specific data.
- **Q5 (GraphRAG)** uses three endpoints: `/extract-graph`, `/graph-query`, and `/community-summary`.
  Submit just the base URL; the grader knows which path to call.
- **CORS errors from the grader:** already handled — `CORSMiddleware` allows all origins and methods.
- **Cost:** the in-memory cache means repeated grader calls are free after the first hit.
  Check usage at `https://aipipe.org/usage` if worried.
- **429 / rate limit from AIPipe:** wait a few seconds and re-Check; the cache prevents re-paying for the same input.
- **Render cold starts:** the free tier spins down after inactivity. The first request after sleep may take ~30 seconds.
  Just re-Check if you get a timeout.
